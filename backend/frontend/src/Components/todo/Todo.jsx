  ...
  const fetchTodos = async () => {
    try {
      ...
      const response = await axios.get(
        `${window.location.origin}/api/v1/todos/alltodos`,
        {
          headers: {
  ...
  const toggleTodo = async (todoId) => {
    ...
    try {
      const response = await axios.put(
        `${window.location.origin}/api/v1/todos/toggle/${todoId}`,
        {
    ...
    if (title && description) {
      try {
        const res = await axios.post(
          `${window.location.origin}/api/v1/todos/add`,
          {
            title: title,
  ...
  const deleteTodo = async (todoId) => {
    try {
      const res = await axios.delete(
        `${window.location.origin}/api/v1/todos/delete/${todoId}`,
        {
          headers: {
            Authorization: "Bearer " + localStorage.getItem("token"),
          },
  ...
  const updateTodo = async (updatedTodo) => {
    try {
      const res = await axios.put(
        `${window.location.origin}/api/v1/todos/update/${updatedTodo._id}`,
        {
